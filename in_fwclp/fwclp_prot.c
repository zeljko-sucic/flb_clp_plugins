...
static int send_ack(struct flb_input_instance *in, struct fwclp_conn *conn,
...
static int fwclp_process_forward_mode_entry(
...
static int fwclp_process_message_mode_entry(
...
static size_t receiver_recv(struct fwclp_conn *conn, char *buf, size_t try_size) {
    size_t off;
    size_t actual_size;
...
static size_t receiver_to_unpacker(struct fwclp_conn *conn, size_t request_size,
...
int fwclp_prot_process(struct flb_input_instance *ins, struct fwclp_conn *conn)
{
    int ret;
    int stag_len;
    int c = 0;
    ...
    size_t all_used = 0;
    struct flb_in_fwclp_config *ctx = conn->ctx;
    struct cmt *cmt;
    struct ctrace *ctr;
    ...
    while (1) {
        ...
        while (ret == MSGPACK_UNPACK_SUCCESS) {
            ...
            tag_suffix = flb_calloc(strlen(conn->connection->remote_host)+HOSTNAME_SIZE+1, sizeof(char));
            strcpy(tag_suffix, "_");
            strcat(tag_suffix, hostname);
            strcat(tag_suffix, "_");
            strcat(tag_suffix, conn->connection->remote_host);
            ...
            if (entry.type == MSGPACK_OBJECT_ARRAY) {
                ...
                for(index = 0 ;
                    ...
                    ret == 0 ;
                    index++) {
                    ret = fwclp_process_forward_mode_entry(
                            conn,
                ...
                /* Process map */
                fwclp_process_message_mode_entry(
                    conn->in, conn,
                    out_tag, flb_sds_len(out_tag),
