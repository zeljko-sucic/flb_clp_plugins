...
int fwclp_conn_event(void *data)
{
    int ret;
    int bytes;
    int available;
    ...
    if (event->mask & MK_EVENT_READ) {
        ...
        if (bytes > 0) {
            ...
            ret = fwclp_prot_process(ctx->ins, conn);
            if (ret == -1) {
                fwclp_conn_del(conn);
                return -1;
            }
...
struct fwclp_conn *fwclp_conn_add(struct flb_connection *connection, struct flb_in_fwclp_config *ctx)
{
    ...
    conn = flb_malloc(sizeof(struct fwclp_conn));
    if (!conn) {
        flb_errno();
    ...
    connection->event.handler = fwclp_conn_event;

    /* Connection info */
    conn->ctx     = ctx;
    conn->buf_len = 0;
    conn->rest    = 0;
    conn->status  = FWCLP_NEW;
...
int fwclp_conn_del_all(struct flb_in_fwclp_config *ctx)
{
    ...
    mk_list_foreach_safe(head, tmp, &ctx->connections) {
        conn = mk_list_entry(head, struct fwclp_conn, _head);
        fwclp_conn_del(conn);
    }
