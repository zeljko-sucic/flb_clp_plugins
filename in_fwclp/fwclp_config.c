...
struct flb_in_fwclp_config *fwclp_config_init(struct flb_input_instance *i_ins)
{
    char tmp[16];
    int ret = -1;
    const char *p;
    struct flb_in_fwclp_config *config;

    config = flb_calloc(1, sizeof(struct flb_in_fwclp_config));
    if (!config) {
        flb_errno();
        return NULL;
    }
...
int fwclp_config_destroy(struct flb_in_fwclp_config *config)
{
    if (config->log_encoder != NULL) {
